...
D-025  2026-09-18  P0-04  The Blazor client publishes with `InvariantGlobalization=true` and `BlazorWebAssemblyLoadAllGlobalizationData=false`; the Api keeps `InvariantGlobalization=false`.
...
D-026  2026-09-18  P0-04  `index.html` references `_framework/blazor.webassembly.js` by its stable path; `OverrideHtmlAssetPlaceholders` is false and the SDK's `#[.{fingerprint}]` placeholders must not be reintroduced.
...
D-027  2026-09-18  P0-04  Blazor WebAssembly stands (D-001 confirmed); no Razor Pages fallback. Trimming stays at the SDK default trim mode, not `TrimMode=full`.
...
D-028  2026-09-18  P0-05  `ISeasonWeekSource` is async and lives in Domain; the 2026 fixture calendar is code, not JSON; `GET /api/seasons/{year}/weeks` answers 404 for a season the calendar does not know.
