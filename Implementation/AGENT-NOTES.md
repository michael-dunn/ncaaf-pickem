...
- **Add a one-shot** (a due time that lives in the database, like the week lock or the Saturday reminder): implement `IOneShotJob` — `Name`, `GetDueAsync(nowUtc, ct)` returning `OneShotOccurrence(Key, DueUtc)`, and `RunAsync(occurrence, ct)` — then `services.AddOneShotJob<YourJob>();`. This is the card's `IOneShotScheduler` (D-029). Answer the naive query and stay stateless: the tick drops occurrences dated after `nowUtc` and skips ones already recorded.
- Idempotency is the `JobRuns(JobName, ScheduledForUtc)` unique index. The tick inserts the row **before** running and treats a duplicate key as "someone else has this one". A one-shot's row is `JobName = "{Name}:{Key}"`, so name + `:` + key must fit 60 characters. **Write every job to be safely re-runnable** (D-028): a crash between claim and finish, or the catch-up window, can offer the same occurrence again.
...
- `HeartbeatJob` (`*/5 * * * *`) is proof of life and owns no table. `GET /api/admin/data-status` shows the last 50 runs; scope an admin route with `RequireAnyLeagueCommissioner()` (403, not 404, for a non-commissioner — D-030).
