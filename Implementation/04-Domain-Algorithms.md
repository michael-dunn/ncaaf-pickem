...
Nightly safety net: `Infrastructure/Jobs/NightlyRescoreJob` (`30 4 * * *` Eastern) recomputes every locked week that is not complete, plus any set whose `IsComplete` disagrees with its own `WeekResults.IsWeekComplete` (D-131). It exists because a handler that throws is logged and skipped rather than retried (D-046), so the event pipeline alone is not enough to keep `WeekResults` true.
