...
- Build: `dotnet publish src/NcaafPickEm.Api -c Release` (the hosted app: Api serves the Web
  ...
  `InvariantGlobalization=true` (D-023), Brotli + gzip precompression on.
...
No ICU data ships at all (`InvariantGlobalization=true`, D-023); that alone removed roughly 1.5 MB
...
**Conclusion: proceed with Blazor WASM. D-001 stands** (logged as D-025). The fallback trigger is
...
1. The hosted app did not boot at all before this task. `index.html` shipped the SDK's literal
   ...
   `_framework/blazor.webassembly.js` path and turning the rewrite off (D-024).
